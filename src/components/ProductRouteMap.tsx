...
// Phase 11 — workspace Map / Route page. Renders REAL route context from the job's uploaded GIS_ROUTE
// (.kmz/.kml) as a self-contained SVG polyline — NO external map tiles, NO library, NO API key, NO
// geocoding. It draws ONLY coordinates the backend parsed from the file (honest empty states otherwise),
// and never claims a redline map overlay: redline geometry is pixel-only and lives on the source PDF pages.
// (A tiled basemap with street labels, v1-style Leaflet+OSM, is a deferred follow-up — it needs the
// external tile network, so it is intentionally NOT added here.)
 ...
 *  fit + centered, north up. Returns a screen-space mapper or null when there is nothing to draw. */
function makeProjector(features: GisRouteView['features']) {
  ...
  const k = Math.max(0.05, Math.cos((meanLat * Math.PI) / 180)); // lon degrees -> ~equal distance
...
export function ProductRouteMap({ jobId, refreshKey }: { jobId: string; refreshKey?: string }) {
  ...
  const project = view && view.present ? makeProjector(view.features) : null;
  ...
  return (
    <Card>
      ...
      <p className="mt-1 text-sm text-ink-3">
        ...
        external map tiles or geocoding. Redline geometry is pixel-only and is shown on the source PDF pages
        in the Redlines section; a geo-referenced map overlay awaits georeference.
      </p>
      ...
      {!error && !loading && view && !view.present && (
        <div className="mt-3 rounded-lg border border-line bg-paper p-4 text-sm text-ink-3">
          ...
          {view.reason === 'NO_GIS_ROUTE_UPLOADED' && (
            <span className="ml-1">Upload a <span className="font-mono">.kmz</span> / <span className="font-mono">.kml</span> in the Uploads section to see the route here.</span>
          )}
        </div>
      )}

      {!error && !loading && view && view.present && project && view.features.length > 0 && (
        <>
          <div className="mt-3 overflow-hidden rounded-lg border border-line bg-white">
            <svg viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} className="h-auto w-full" role="img"
              ...
              {view.features.map((f, fi) => {
                if (f.coordinates.length === 0) return null;
                const pts = f.coordinates.map((p) => project(p));
                if (f.type === 'Point') {
                  const [x, y] = pts[0];
                  return <circle key={fi} cx={x} cy={y} r={5} fill="#D85408" stroke="#fff" strokeWidth={1.5} />;
                }
                const d = pts.map(([x, y]) => `${x.toFixed(1)},${y.toFixed(1)}`).join(' ');
                if (f.type === 'Polygon') {
                  return <polygon key={fi} points={d} fill="rgba(244,100,14,0.10)" stroke="#D85408" strokeWidth={1.5} />;
                }
                return (
                  <g key={fi}>
                    <polyline points={d} fill="none" stroke="#D85408" strokeWidth={2.5}
                              strokeLinejoin="round" strokeLinecap="round" />
                    <circle cx={pts[0][0]} cy={pts[0][1]} r={4} fill="#0a7d33" />
                    <circle cx={pts[pts.length - 1][0]} cy={pts[pts.length - 1][1]} r={4} fill="#b01515" />
                  </g>
                );
              })}
            </svg>
          ...
          <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-ink-3">
            <span><span className="inline-block size-2 rounded-full bg-[#0a7d33]" /> start</span>
            <span><span className="inline-block size-2 rounded-full bg-[#b01515]" /> end</span>
          ...
          <ul className="mt-2 divide-y divide-line border-t border-line text-sm">
            {view.features.slice(0, 12).map((f, fi) => (
            ...
            {view.featureCount > 12 && (
              <li className="py-1.5 text-xs text-ink-3">+ {view.featureCount - 12} more feature(s)</li>
            )}
          </ul>
